...
public class dfControlInspector : Editor
{
	...
	{ 
		...
		targetObjectCount = 
			Selection.gameObjects
				.Select( c => c.GetComponent<dfControl>() ).Count(c => c != null);
	...
	private void OnInspectCommonProperties( dfControl control )
	{
		...
		using( dfEditorUtil.BeginGroup( "Behavior" ) )
		{
			...
			var canChangeTransformManually = EditorGUILayout.Toggle( "Can Change Transform Manually", control.canChangeTransformManually );
			if ( canChangeTransformManually != control.canChangeTransformManually ) {
				dfEditorUtil.MarkUndo( control, "Toggle 'Can Change Transform Manually' property" );
				control.canChangeTransformManually = canChangeTransformManually;
			}
		}
	...
	private void createControlMoveHandle( dfControl control )
	{
		...
		if( control.HitTest( HandleUtility.GUIPointToWorldRay( mousePosition ) ) != null )
		{
