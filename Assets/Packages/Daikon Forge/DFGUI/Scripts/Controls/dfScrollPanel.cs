...
public class dfScrollPanel : dfControl
{
	...
	private bool hasInvalidatedControlLayout = false;
	...
	protected internal override void OnMouseMove( dfMouseEventArgs args )
	{
		if( args is dfTouchEventArgs || isMouseDown )
		{
			...
			if( !args.Used && ( args.Position - touchStartPosition ).magnitude > 5 )
			{
				...
				delta.x = wheelDirection == dfControlOrientation.Horizontal
					? screenSize.x * ( delta.x / renderCamera.pixelWidth )
					: 0f;

				delta.y = wheelDirection == dfControlOrientation.Vertical
					? screenSize.y * ( delta.y / renderCamera.pixelHeight )
					: 0f;
	...
	private void onChildControlInvalidatedLayout()
	{
		...
		hasInvalidatedControlLayout = true;
	}

	private void UpdateInvalidatedLayout() {

		if ( !hasInvalidatedControlLayout ) {
		...
		hasInvalidatedControlLayout = false;
	}
