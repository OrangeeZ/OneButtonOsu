...
public abstract class dfControl : MonoBehaviour, IDFControlHost, IComparable<dfControl>
{
	...
	/// <summary>
	/// This enables the runtime checks for transform state change
	/// </summary>
	public bool canChangeTransformManually = false;
	...
	{
		get { return this.color.a / 255f; }
		set
		{
			value = Mathf.Max( 0, Mathf.Min( 1, value ) );
			var alpha = this.color.a / 255f;
	...
	protected internal bool Signal( GameObject target, string eventName, object[] args )
	{
		...
		for( int i = 0; i < components.Length; i++ )
		{
			...
			if( handledByComponent )
			{
				...
				var value = returnValue as IEnumerator;
				if( value != null ) {

					component.StartCoroutine( value );
				}
	...
	public Vector3[] GetCorners()
	{
		...
		var scaledSize = size * p2u;

		var right = transform.TransformDirection( new Vector3( scaledSize.x, 0 ) );
		var down = transform.TransformDirection( new Vector3( 0, -scaledSize.y ) );

		var upperLeft = transform.TransformPoint( pivot.TransformToUpperLeft( scaledSize ) );
		var upperRight = upperLeft + right;//new Vector3( size.x, 0 );
		var bottomLeft = upperLeft + down;//new Vector3( 0, -size.y );
		var bottomRight = upperRight + down;//new Vector3( 0, -size.y );
		...
		cachedCorners[ 0 ] = ( upperLeft  );
		cachedCorners[ 1 ] = ( upperRight );
		cachedCorners[ 2 ] = ( bottomLeft );
		cachedCorners[ 3 ] = ( bottomRight );
	...
	public Rect GetScreenRect() {
		...
		//if ( screenRectCache == null ) {
			...
			//screenRectCache = result;

			return result;
		//}

		//return screenRectCache.Value;
	}
	...
	public dfGUIManager GetManager()
	{
		...
		//}
		cachedManager = GetComponentsInParent<dfGUIManager>().FirstOrDefault();
	...
	protected internal virtual Plane[] GetClippingPlanes()
	{
		...
		var left = -right;//transform.TransformDirection( Vector3.left );
		var up = transform.TransformDirection( Vector3.up );
		var down = -up;//transform.TransformDirection( Vector3.down );
	...
	/// </summary>
	[HideInInspector]
	protected internal virtual void OnPositionChanged( Vector3 newPosition ) {


		screenRectCache = null;
...
//		}
	...
	public virtual void OnDrawGizmos()
	{
		...
		if( this.HotZoneScale != Vector2.one )
		{
		...
		if ( canChangeTransformManually 
		  ...
		  || !Application.isPlaying
			...
			if ( cachedTransform.hasChanged ) {
				...
				var newScale = cachedTransform.localScale;
				var newRotation = cachedTransform.localRotation;

				// Control bounds are no longer valid
				cachedBounds = null;

				// If the control's scale has changed, need to rebuild render data
				if ( cachedScale != newScale ) {
				...
				if ( !Vector3Utility.ApproximateEqual( cachedPosition, newPosition ) ) {
				...
				if ( !isControlInvalidated && cachedRotation != newRotation ) {
	...
	public T AddPrefab<T>( T prefab ) where T: Behaviour {
		...
		var result = Instantiate( prefab ) as T;
	...
	private void initializeControl()
	{
		...
		if( cachedParentTransform != transformParent )
		{
	...
	private void setRelativePosition( ref Vector3 value )
	{
		...
		if( parent != null )
		{
			...
			if( ( newPos - cachedPosition ).sqrMagnitude >= float.Epsilon )
			{
		...
		if( !Vector3Utility.ApproximateEqual( newPosition, cachedPosition ) )
		{
	...
	private static Vector3 closestPointOnLine( Vector3 start, Vector3 end, Vector3 test, bool clamp )
	{
		...
		var c = test - start;				// Vector from a to Point
		var d = endToStart.magnitude;	// Length of the line segment
		var v = endToStart / d;	// Unit Vector from a to b
		var t = Vector3.Dot( v, c );			// Intersection point Distance from a
