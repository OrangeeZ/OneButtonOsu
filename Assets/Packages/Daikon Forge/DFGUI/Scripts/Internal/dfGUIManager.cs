...
public class dfGUIManager : MonoBehaviour, IDFControlHost, IComparable<dfGUIManager>
{
	...
	public dfControl HitTest( Vector2 screenPosition )
	{
		...
		for( var i = controlCount - 1; i >= 0; i-- )
		{
			...
			var hitPosition = control.HitTest( ray );
			if ( hitPosition == null )
				continue;
			...
			if ( isInsideClippingRegion( hitPosition.Value, control ) )
			{
				return control;
			}
	...
	public void Render()
	{
		...
		//{
			...
			if ( renderFilter.sharedMesh == null ) {
				...
				renderFilter.sharedMesh = new Mesh();
			}
			else {
			...
			renderFilter.sharedMesh.CombineMeshes( new[] { new CombineInstance { mesh = mesh, transform = transform.worldToLocalMatrix } }, mergeSubMeshes: false, useMatrices: true );
		
			mesh = renderFilter.sharedMesh;
	...
	private dfList<dfControl> getTopLevelControls()
	{
		...
		try
		{
			...
			for( int i = 0; i < controlCount && controls.Count < childCount; i++ )
			{
	...
	private dfRenderData compileMasterBuffer()
	{
		...
		try
		{
			...
			//masterBuffer.Transform = localToWorldMatrix;
	...
	private Material[] gatherMaterials()
	{
		...
		try
		{
			...
			for( int i = 0; i < drawCallBuffers.Count; i++ )
			{
				...
				if ( drawCallMaterial.mainTexture != buffer.Material.mainTexture ) {
					
					drawCallMaterial.mainTexture = buffer.Material.mainTexture;
				}

				var targetShader = buffer.Shader ?? drawCallMaterial.shader;

				if ( drawCallMaterial.shader != targetShader ) {
	...
	private void renderControl( ref dfRenderData buffer, dfControl control, uint checksum, float opacity )
	{
		...
		var absolutePosition = control.GetAbsolutePosition();
		var size = control.Size;
		var screenSize = cachedScreenSize;

		if ( absolutePosition.x + size.x < 0 || absolutePosition.x > screenSize.x ) {

			return;
		}

		if ( absolutePosition.y + size.y < 0 || absolutePosition.y > screenSize.y ) {
	...
	private bool processRenderData( ref dfRenderData buffer, dfRenderData controlData, ref Bounds bounds, uint checksum, dfTriangleClippingRegion clipInfo, ref bool wasClipped )
	{
		...
		{
			if( controlData.Material != buffer.Material )
			{
				needNewDrawcall = true;
			}
			else if( controlData.Material.mainTexture != buffer.Material.mainTexture )
			{
				needNewDrawcall = true;
			}
			else if( buffer.Shader != controlData.Shader )
			{
				needNewDrawcall = true;
			}
		}
		...
		if( needNewDrawcall )
		{
			...
			var newShader =controlData.Shader ?? controlData.Material.shader;

			if ( newShader != buffer.Material.shader ) {

				buffer.Material.shader = newShader;
			}
		}
