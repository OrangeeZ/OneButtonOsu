...
internal class dfTempArray<T>
{
	...
	private static LinkedList<T[]> cache = new LinkedList<T[]>();
		...
		lock( cache ) {
			...
			for( int i = 0; i < cache.Count; i++ ) {
				...
				if( list.Length == length )
				{
					...
					if( i > 0 )
					{
						cache.Remove( enumerator );
						cache.AddFirst( enumerator );
					}
