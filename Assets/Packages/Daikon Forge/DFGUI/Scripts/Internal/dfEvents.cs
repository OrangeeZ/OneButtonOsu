...
public class dfTouchEventArgs : dfMouseEventArgs
{
	...
	/// <param name="ray">The <see cref="Ray"/> from the screen mouse location through the <paramref name="Source"/> control</param>
	public dfTouchEventArgs( dfControl Source, dfTouchInfo touch, Ray ray, float dpiRate )
		: base( Source, dfMouseButtons.Left, touch.tapCount, ray, touch.position, 0f )
	{
		...
		var deltaRate = Time.deltaTime / touch.deltaTime;
		MoveDelta = touch.deltaPosition;// * ( float.IsNaN( deltaRate ) ? 1f : deltaRate );
		MoveDelta *= dpiRate;
	}
	...
	public dfTouchEventArgs( dfControl source, List<dfTouchInfo> touches, Ray ray, float dpiRate )
		: this( source, touches.First(), ray, dpiRate )
	{
