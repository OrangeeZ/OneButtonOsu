...
public class dfInputManager : MonoBehaviour
{
	...
	[SerializeField] 
	protected bool useDpiForTouch = true;

	[SerializeField]
	protected int ethalonDpi = 300;

	protected float dpiRate {
		get { return useDpiForTouch ? Screen.dpi / ethalonDpi : 1f; }
	}
	...
	public void OnEnable()
	{
		...
		if( this.adapter == null )
		{
			...
			var inputAdapter = GetComponents<MonoBehaviour>().OfType<IInputAdapter>().FirstOrDefault();

			// Use the replacement if found, otherwise use the default adapter
			this.adapter = inputAdapter ?? new DefaultInput();
	...
	private bool processTouchInput()
	{
		...
		if( touchInputSource == null )
		{
			...
			// touch input source or assign default one
			touchInputSource = GetComponents<dfTouchInputSourceComponent>()
				.OrderByDescending( x => x.Priority )
				.FirstOrDefault( each => each != null && each.enabled )
				.Return( _ => _.Source, dfMobileTouchInputSource.Instance );
		}
	...
	private class TouchInputManager
	{
		...
		internal void Process( Transform transform, Camera renderCamera, IDFTouchInputSource input, bool retainFocusSetting )
		{
			...
			foreach ( var touch in touches ) {
				...
				var info = new TouchRaycast( controlUnderMouse, touch, ray, manager.dpiRate );
				...
				var processed = tracked.Any( t => t.Process( info ) );

				if( !processed && controlUnderMouse != null )
				{

					if( tracked.All( x => x.control != controlUnderMouse ) )
					{
			...
			public bool Process( TouchRaycast info )
			{
				...
				if( !touches.ContainsKey( info.FingerID ) )
				{
					...
					if( info.Phase == TouchPhase.Began )
					{
						...
						var multiTouchArgs = new dfTouchEventArgs( control, activeTouches, info.ray, manager.dpiRate  );
		...
		private class TouchRaycast
		{
			...
			public TouchRaycast( dfControl control, dfTouchInfo touch, Ray ray, float dpiRate )
			{
				this.control = control;
				this.touch = touch;
				this.ray = ray;
				this.dpiRate = dpiRate;
			...
			{
				var args = new dfTouchEventArgs( touch.control, touch.touch, touch.ray, touch.dpiRate );
				return args;
			}
